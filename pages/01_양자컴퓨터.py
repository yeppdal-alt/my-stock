...
QUANTUM_TICKERS = {
    ...
    "Intel (INTC)": "INTC",
    # 일본
    "Toshiba (6502.T)": "6502.T",
    "Fujitsu (6702.T)": "6702.T",
}

PURE_PLAY_NAMES = {
    "IonQ (IONQ)", "Rigetti Computing (RGTI)", "D-Wave Quantum (QBTS)",
    "Quantum Computing Inc (QUBT)", "Arqit Quantum (ARQQ)",
    "Quantum-Si (QSI)", "SEALSQ (LAES)",
}

BENCHMARKS = {
    "Defiance Quantum ETF (QTUM)": "QTUM",
    "나스닥 100 (^NDX)": "^NDX",
    "S&P 500 (^GSPC)": "^GSPC",
}
...
def get_fx_rate(currency: str) -> float:
    """1 USD = ? <currency>. USD는 1.0. 조회 실패 시 1.0으로 대체(환산 생략)."""
    if not currency or currency == "USD":
        return 1.0
    try:
        df = yf.Ticker(f"{currency}=X").history(period="5d")
        if not df.empty:
...
def load_fundamentals(ticker: str) -> dict:
    ...
    return {
        ...
        "총현금_raw": info.get("totalCash"),
        "PER(trailing)": info.get("trailingPE"),
        "PS(trailing)": info.get("priceToSalesTrailing12Months"),
        "PBR": info.get("priceToBook"),
        "매출성장률(YoY)": info.get("revenueGrowth"),
        "영업이익률": info.get("operatingMargins"),
        "순이익률": info.get("profitMargins"),
        "베타": info.get("beta"),
        "52주 최고_raw": info.get("fiftyTwoWeekHigh"),
        "52주 최저_raw": info.get("fiftyTwoWeekLow"),
    }
...
selected_names = st.sidebar.multiselect(
    "분석할 양자컴퓨터 관련 종목",
    options=list(QUANTUM_TICKERS.keys()),
    default=["IonQ (IONQ)", "Rigetti Computing (RGTI)", "D-Wave Quantum (QBTS)", "IBM (IBM)", "NVIDIA (NVDA)"],
)
...
tech_name = st.sidebar.selectbox(
    "기술적 분석 대상 종목",
    options=selected_names if selected_names else list(QUANTUM_TICKERS.keys()),
)
...
with col_title:
    st.title("⚛️ 양자컴퓨터 주식 전문 분석")
    st.caption("IonQ, Rigetti, D-Wave, IBM 등 양자컴퓨팅 밸류체인 핵심 종목 · 밸류에이션 · 기술적 분석 · 상대성과 · 상관관계")
with col_time:
    st.markdown(
...
st.info(
    "🧪 **섹터 특성**: 양자컴퓨팅 산업은 대부분 상용화 초기 단계로, 순수 플레이(IonQ·Rigetti·D-Wave 등)는 "
    "매출이 미미하거나 적자인 경우가 많습니다. PER이 N/A로 뜨는 것은 오류가 아니라 적자 상태이기 때문이며, "
    "이런 종목은 PER 대신 **P/S(매출 대비 시가총액)**와 **보유 현금(런웨이)**을 함께 참고하는 것이 좋습니다."
)
...
selected_tickers = {name: QUANTUM_TICKERS[name] for name in selected_names}
...
with tab1:
    ...
    with st.spinner("펀더멘털 데이터 불러오는 중..."):
        for name, ticker in selected_tickers.items():
            ...
            cash_usd_m = (cash_raw / fx_rate / 1e6) if isinstance(cash_raw, (int, float)) else None

            rows.append({
                "종목": name,
                "티커": ticker,
                "유형": "순수 양자플레이" if name in PURE_PLAY_NAMES else "대형 기술주",
                "원통화": currency,
                "현재가($)": fmt_num(price_usd),
                "시가총액(B$)": fmt_num(mc_usd_b),
                "총현금(M$)": fmt_num(cash_usd_m, 1),
                "PER(TTM)": fmt_num(f["PER(trailing)"]),
                "P/S(TTM)": fmt_num(f["PS(trailing)"]),
                "PBR": fmt_num(f["PBR"]),
                ...
                "_ps_raw": f["PS(trailing)"],
                "_opmargin_raw": f["영업이익률"],
                "_revgrowth_raw": f["매출성장률(YoY)"],
            })

    df_val = pd.DataFrame(rows)
    display_cols = ["종목", "티커", "유형", "원통화", "현재가($)", "시가총액(B$)", "총현금(M$)",
                     "PER(TTM)", "P/S(TTM)", "PBR", "매출성장률", "영업이익률", "순이익률", "베타"]
    ...
    if fx_used:
        fx_note = " · ".join([f"1$ ≈ {rate:,.1f}{cur}" for cur, rate in fx_used.items()])
        st.caption(f"💱 원화·엔화 등 비($) 종목은 실시간 환율로 달러 환산했습니다 ({fx_note}). "
                   f"PER·PBR·P/S 등 비율 지표는 통화 환산이 필요 없어 원본 값 그대로입니다.")
    if latest_fetch:
    ...
    with col1:
        ...
        fig_op.update_layout(title="영업이익률 비교 (%) · 음수면 적자", height=400,
                              margin=dict(l=10, r=10, t=50, b=10))
        st.plotly_chart(fig_op, use_container_width=True)

    with col2:
        fig_ps = go.Figure(go.Bar(
            x=df_val["종목"], y=df_val["_ps_raw"],
            marker_color="rgba(255,140,0,0.75)",
        ))
        fig_ps.update_layout(title="P/S(TTM) 비교 · 낮을수록 매출 대비 저평가", height=400,
                              margin=dict(l=10, r=10, t=50, b=10))
    ...
    st.caption("※ PER·P/S가 N/A인 경우 적자 기업이거나 데이터 미제공 종목입니다. 순수 양자플레이는 대부분 P/S로 평가하는 것이 더 적절합니다.")

# ----------------------------------------------------
# TAB 2: 기술적 분석 (캔들스틱 + MA + RSI + MACD)
# ----------------------------------------------------
with tab2:
    tech_ticker = QUANTUM_TICKERS[tech_name]
    ...
    if df.empty:
        ...
        if tech_name in PURE_PLAY_NAMES:
            st.caption("⚠️ 순수 양자컴퓨팅 종목은 변동성이 매우 크고 뉴스/이벤트에 민감하게 반응하는 경향이 있습니다. "
                        "기술적 지표만으로 판단하기보다 재무 건전성(현금 보유량)을 함께 확인하세요.")
...
with tab3:
    ...
    st.caption("※ 순수 양자플레이 종목은 소형주 특성상 급등락이 커서 그래프 스케일이 다른 종목을 압도할 수 있습니다.")
...
st.caption(
    ...
    "양자컴퓨팅 관련 종목은 변동성이 매우 크므로 투자 참고용으로만 활용하시고, 투자 판단의 책임은 본인에게 있습니다."
)
